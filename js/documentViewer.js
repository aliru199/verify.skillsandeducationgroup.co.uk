...
var DocumentViewer = (function () {
  ...
  const templates = {
    panel:
      "<div id='documentViewerDrawer' class='kt-quick-panel'>" +
      ...
      "</div>",
    loadingImage:
      "<img id='documentLoader' src='/images/loading.gif' alt='Loading'/>",
  };
  ...
  function addStyleSheet() {
    ...
    $("head").append(
      "<link id='documentViewerStyles' href='/css/documentViewer.css' type='text/css' rel='stylesheet' />"
    );
  }
  ...
  function setSessionRecordIds() {
    session.dataTableDocumentIds = session.dataTable
  ...
  function changePage(direction) {
    ...
    session.dataTable.one("kt-datatable--on-layout-updated", function () {
      ...
      if (direction === pageDirection.PREVIOUS) {
        pageChangedRenderId =
          session.dataTableDocumentIds[session.dataTableDocumentIds.length - 1];
      }
  ...
  function handleDocumentResult(result) {
    ...
    const dataPath = abp.utils.formatString(
      "{0}viewer/{1}/{2}",
      abp.appPath,
  ...
  function getUrlPath(recordTypeCodes) {
    if (config.isPublishWizard !== null) {
      dataPath = abp.utils.formatString(
        "{0}viewer/data/{1}/{2}/{3}/{4}",
        abp.appPath,
        session.electronic,
        config.accessType,
        session.recordId,
        config.isPublishWizard
      );
    } else {
      dataPath = abp.utils.formatString(
        "{0}viewer/data/{1}/{2}/{3}",
  ...
  function renderDocument() {
    ...
    var recordTypeCodes = !("WizardSearchFilter" in window)
      ? []
      : WizardSearchFilter?.getCodes();
    ...
    $.get(dataPath)
      ...
      .fail(function (response) {
        ...
        const message = errorMessage
          ...
          : LWithSource(
              localisationSource,
              "Exception:DocumentViewer:Data:Message"
            );
        const title = LWithSource(
          localisationSource,
          "Exception:DocumentViewer:Data:Title"
        );
        const noAccessMessage = LWithSource(
          localisationSource,
          "Exception:Viewer:NoAccess:Message"
        );
        ...
        if (message === noAccessMessage && accessErrorModal) {
          $("#PurchaseAccess").attr(
            "href",
            "/store?documentId=" + session.recordId
          );
  ...
  function renderDropdown() {
    ...
    const select = $("<select id='documentSelector'></select>").addClass(
      "form-control"
    );
    $.each(session.documents, function (index, value) {
      const expiredText = LWithSource(
        localisationSource,
        "DocumentViewer:Expired"
      );
      const recordTypeCode = value.isElectronic
        ? abp.utils.formatString("E-{0}", value.recordTypeCode)
      ...
      select.append(
        $("<option></option>")
          ...
          .attr(
            "selected",
            value.id === session.recordId &&
              value.isElectronic === session.electronic
          )
          .text(text)
      );
    });
  ...
  function renderExpiryMessage() {
    ...
    const warningText = LWithSource(
      localisationSource,
      "Warning:DocumentViewer:DocumentExpired",
      formattedExpiryDate
    );
    const warning = abp.utils.formatString(
      '<div class="alert alert-danger m-3 text-center" role="alert">{0}</div>',
      warningText
    );
  ...
  function addViewerCard() {
    ...
    const content = $(
      abp.utils.formatString(templates.panel, templates.loadingImage)
    );
  ...
  function setViewerIFrame(dataPath) {
    ...
    $(containerSelector).html(
      "<iframe src='" + dataPath + "' class='w-100 h-100'></iframe>"
    );
  }
