...
$.validator.methods.required = function (value, element, param) {
  ...
  if (
    ...
    element.classList.contains("summernote") &&
...
var formatIcon = function (icon) {
  return $(
    '<span><i class="far fa-' + icon.text + '" />  ' + icon.text + "</span>"
  );
};
...
$("body").on("click", "[data-clone]", function (event) {
  ...
  var clonedNode = $(selector + ":first")
    .clone()
    .attr("id", "row-" + nodeCount);
...
function initSelect2Remote(selector) {
  ...
  $(selector + " .m-select2-remote:not(.select2-hidden-accessible)").each(
...
$(".m-datetimepicker, .kt-datetimepicker")
  .datetimepicker({
    todayHighlight: true,
    autoclose: true,
...
function initDatepicker(container) {
  $(container)
    .find(".m-datepicker, .kt-datepicker")
    .datetimepicker({
      format: "dd/mm/yyyy",
      todayHighlight: true,
      autoclose: true,
    ...
    .on("changeDate", function (e) {
      if (e.date === undefined) {
        $(e.target).data(
          "value",
          moment($(e.target).val(), "DD/MM/YYYY").format("YYYY-MM-DD")
        );
      } else if (e.date === null) {
        $(e.target).data("value", "");
      } else {
...
$(".m-datepicker, .kt-datepicker").keydown(function (e) {
  ...
  if (keyCode !== 9) {
    //prevent keyboard trap and allow tabbing
    e.preventDefault();
    return false;
  }
    ...
    .each(function (index, input) {
      ...
      if (name !== undefined) {
        ...
        if (field.hasClass("custom-data") && customData[name] === undefined) {
          if ($(input).is(":radio")) {
            customData[name] = $(
              abp.utils.formatString("input[name='{0}']:checked", name)
            ).val();
          } else if ($(input).is(":checkbox")) {
            $(abp.utils.formatString("input[name='{0}']:checked", name)).each(
              function () {
                customData[name] =
...
function initValidation(formId, ruleSet, addEvents) {
  ...
  if (addEvents === undefined) addEvents = true;
  ...
  if (addEvents) {
    $(abp.utils.formatString("[data-form='{0}']", formId)).on(
      "click",
      function (e) {
        e.preventDefault();
        button = $(this);
  ...
  var validator = $form.validate({
    ...
    highlight: function (input) {
      ...
      if (!invalidFields.includes(id)) invalidFields.push(id);
    },
  ...
  $form.submit(function (e) {
    ...
    if (!$form.valid()) {
      $.each(tabErrors, function (index, tabId) {
    ...
    const regex = new RegExp(
      "[^a-zA-Z0-9-&()/\\\\_ -\u0600-\u06ff\u0750-\u077f\ufb50-\ufbc1\ufbd3-\ufd3f\ufd50-\ufd8f\ufd50-\ufd8f\ufe70-\ufefc\uFDF0-\uFDFD\u0370-\u03ff\u1f00-\u1fff\u0400-\u04FFÀ-ÿœı ' -]"
    );
    ...
    if (submitHandle && submitEvent) {
      abp.message.warn(
        "You cannot use both handler and event at the same time.",
        "Form configuration"
      );
      return;
    }
...
function localeCurrencyFormat(num) {
  return (
    abp.setting.get("App.System.Currency.Symbol") +
    num.toLocaleString(abp.localization.currentCulture.name, {
...
function initSessionWatch() {
  $.sessionTimeout({
    ...
    redirAfter:
      (parseInt(abp.setting.get("Session.Timeout.Redirect")) +
...
$("body").on("click", "[data-action='massdelete']", function (event) {
  ...
  if (!handler) {
    ...
    abp.message.confirm(
      L("DeleteMassRecordMessage"),
      L("DeleteMassRecordTitle"),
      function (isConfirmed) {
        if (isConfirmed) {
...
$("body").on("click", "[data-action='release']", function (event) {
  ...
  if (!handler) {
    ...
    abp.message.confirm(
      L("ReleaseRecordsMessage"),
      L("releaseRecordsTitle"),
      function (isConfirmed) {
        if (isConfirmed) {
          executeFunctionByName(handler, window);
        }
...
function UpdateHeaderProfileImage(userId) {
  ...
  $(
    ".kt-header__topbar-item--user .kt-badge--username, .kt-user-card .kt-badge--username"
  ).addClass("kt-hidden");
}
...
function initRangeDatePicker(container) {
  ...
  $(elements)
    ...
    .on("apply.daterangepicker", function (ev, picker) {
      $(this).val(
        `${picker.startDate.format(dateFormat)} - ${picker.endDate.format(
          dateFormat
        )}`
      );
